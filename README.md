...
It works. You move on. Three weeks later the same problem comes back, and you've completely forgotten this command exists. So you Google it again, or ask ChatGPT again, and rebuild something you already solved once.
...
docker run -d --name pg-db -e POSTGRES_PASSWORD=secret -p 5432:5432 postgres:15

# 2. Save it immediately with a quick description
memora save "start local postgres database"

# 3. Weeks later, run it again by just describing what you need
...
| `memora add "desc" -- cmd` | Add a command manually | `memora add "restart docker" -- docker compose restart` |
| `memora list` | Show everything you've saved | `memora list` |
| `memora delete [id]` | Delete a saved command | `memora delete H8Q7xxe3` |
